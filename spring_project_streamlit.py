...
with stream.echo(code_location='below'):
    ...
    def main():
        ...
        'Для избежания ошибки "заглядывания в будущее" отсортируем данные по времени и приведем их к формату, пригодному для машинного обучения.'
        ml_df = ml_df.sort_values('pickup_datetime')
        ml_df['pickup_datetime'] = ml_df['pickup_datetime'].map(dt.datetime.toordinal)

        'Далее я оставлю лишь словесное описание шагов, а сам код Вы можете посмотреть ниже (для визуального восприятия он не нужен):'
        ...
        'Как мы видим, регрессия дала плохие результаты, что говорит о плохом выборе признаков. Плохой результат - тоже результат, возможно, какие-то из признаков не совсем осмысленные. Но технически все сделано верно.'
        ...
        'Рассмотрим данные с точки зрения графов. В данном случае данные будут представлять собой набор ребер из двух точек. Если отображать сразу все, то на рисунке будет каша, но если отображать малое количество точек, то по рисункам можно будет делать какие то выводы. Создадим универсальную функцию для построения графов на координатной плоскости.'
        ...
        'Ниже граф для 1000 поездок - каша:('

        make_networks_graph(train_df, 1000)

        'Для примера нарисуем такой граф для 10 поездок.'

        make_networks_graph(train_df, 10)

        '**Промежуточный итог**: Данную функцю можно использовать для дальнейшего анализа.'
        ...
        'Построим график зависмости количества ользователей от часа.'
        print_time_graph(taxi_df, "pickup_datetime_hour")

        'Видим что конец и начало графика почти совпадают. Ночью никто не ездит, утром все едеут на работу, а вечером после 17 - едут домой.'
        ...
        'Построим график зависмости количества пользователей от даты.'
        ...
        'Видим, что координаты явным образмо складываются в очертания районов и островов Нью-Йорка. Проверим эту гипотезу.'
        ...
        '3. Математические способности Python'
        '4. Streamlit'
