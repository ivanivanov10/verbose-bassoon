...
with stream.echo(code_location='below'):
    ...
    '''К заданию приложены очищенные и готовые к исследованию данные, при этом исходные данные взяты из [этого источника](https://www1.nyc.gov/site/tlc/about/tlc-trip-record-data.page), но мы в рамках данного проекта и для чистоты эксперимента возьмем данные из оригинального источника, и потом уже на их основе будем проводить собственный анализ.'''
    ...
    def main():
        ...
        'Далее я оставлю лишь словесное описание шагов, а сам код Вы можете опять же посмотреть ниже. Что мы делаем:'
        ...
        'Как мы видим, анализ дал плохие результаты, что скорее всего говорит о плохом выборе признаков. Плохой результат - тоже результат, возможно, какие-то из признаков можно выкинуть/заменить! Но технически все сделано верно.'
        ...
        'Я проверила, везде ли совпадает данная и посчитанная длительность.'
        stream.dataframe(taxi_df[taxi_df['trip_duration'] != taxi_df['trip_duration_calc']])

        'Как видим, у нас получится пустой датафрейм, значит все совпало. Повезло!'
        ...
        stream.dataframe(taxi_df['vendor_id'].value_counts())
        ...
        'Попробуем использовать эту функцию с конкретными количествами. Ниже видим граф для 1000 поездок: что интересно, явно видны скопления конца поездки, что может быть связано с каким-то популярным местом, которое многие люди посещают. По данному графу можно попробовать сделать гипотезы относительно популярных мест отправки и высадки пассажиров.'
        ...
        '**Промежуточный итог**: Помимо выводов, которые описаны выше, данную функцию можно использовать для дальнейшего анализа каких-то тенденций в данных.'
        ...
        '1. Pandas (продвинутые функции по типу groupby)'
